...
class AlgorithmsTest:
    ...
    def test_algorithms(self):
        ...
        """
        with open(os.path.join(processingTestDataPath(),
                               self.test_definition_file()), 'r',
        ...
        if 'tests' in algorithm_tests and algorithm_tests['tests'] is not None:
            for idx, algtest in enumerate(algorithm_tests['tests']):
                print('About to start {} of {}: "{}"'.format(idx,
                                                             len(
                                                                 algorithm_tests[
                                                                     'tests']),
                                                             algtest['name']))
    ...
    def check_algorithm(self, name, defs):
        ...
        self.assertTrue(ok,
                        'Algorithm failed checkParameterValues with result {}'.format(
                            msg))
        ...
        if expectFailure:
            ...
            results, ok = alg.run(parameters, context, feedback)
            self.assertTrue(ok, 'params: {}, results: {}'.format(parameters,
                                                                 results))
        ...
        try:
            ...
            if param['type'] == 'vrtlayers':
                ...
                for p in param['params']:
                    p['layer'] = self.load_layer(None, {'type': 'vector',
                                                        'name': p['layer']})
                    vals.append(p)
                return vals
            if param['type'] == 'multi':
    ...
    def load_layer(self, param_id, param):
        ...
        self.assertTrue(lyr.isValid(),
                        'Could not load layer "{}" from param {}'.format(
                            filepath, param))
    ...
    def uri_path_join(self, prefix, filepath):
        ...
        if filepath.startswith('ogr:'):
            if not prefix[-1] == os.path.sep:
                prefix += os.path.sep
            filepath = re.sub(r"dbname='", "dbname='{}".format(prefix),
                              filepath)
        else:
    ...
    def check_results(self, results, context, params, expected):
        ...
        for _id, expected_result in expected.items():
            if expected_result['type'] in ('vector', 'table'):
                if 'compare' in expected_result and not expected_result[
                    'compare']:
                    ...
                    try:
                        ...
                        raise KeyError(
                            'Expected result {} does not exist in {}'.format(
                                str(e), list(results.keys()))) from e
                ...
                if len(expected_lyrs) == 1:
                    self.assertLayersEqual(expected_lyrs[0], result_lyr,
                                           compare=compare, pk=pk)
                else:
                    res = False
                    for layer in expected_lyrs:
                        if self.checkLayersEqual(layer, result_lyr,
                                                 compare=compare, pk=pk):
                            res = True
                            break
                    self.assertTrue(res,
                                    'Could not find matching layer in expected results')

            elif 'rasterhash' == expected_result['type']:
                print("id:{} result:{}".format(_id, results[_id]))
                self.assertTrue(os.path.exists(results[_id]),
                ...
                if isinstance(expected_result.get('name'), list):
                    ...
                    for path in expected_result['name']:
                        expected_filepath = self.filepath_from_param(
                            {'name': path})
                        if self.checkFilesEqual(expected_filepath,
                                                result_filepath):
                            break
                    else:
                        expected_filepath = self.filepath_from_param(
